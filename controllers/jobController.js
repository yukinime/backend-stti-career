...
const mapJobPayloadToDb = (p = {}) => {
  ...
  out.requirements = p.qualifications ?? getAliasValue(p, ["requirements", "requirement", "job_requirements", "jobRequirements", "jobRequirement"]);
...
const mapDbRowToApi = (r = {}) => {
  ...
  const computedRange = (r.salary_range ?? "").trim() || (salaryMin || salaryMax ? `${salaryMin} - ${salaryMax}` : "");
...
exports.updateJob = async (req, res) => {
  try {
    ...
    const allowList = ["title", "description", "requirements", "is_active", "verification_status", "company_id", "category_id", "location", "salary_min", "salary_max", "salary_range", "work_type", "work_time"];
    ...
    const reqVal = raw.qualifications ?? getAliasValue(raw, ["requirements", "requirement", "job_requirements", "jobRequirements", "jobRequirement"]);
    if (reqVal !== undefined) {
